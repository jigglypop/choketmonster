...
PokéAPI의 포켓몬 1,025종 자료를 사용하는 팬 게임이다. 현재 코드의 플레이 지도는 **성도·관동·호연·신오·하나·칼로스·알로라**이며, 각 지방의 도시·도로 연결을 근거로 자체 3D 지형과 충돌을 구성했다. 가라르·히스이·팔데아도 지형과 진행 데이터를 작성했으며 전체 모델 검증과 통합 실행을 진행 중이다. 지역별 공개 여부와 모델 검증 범위는 [전 세대 준비 기록](docs/all-generations-readiness-2026-09-16.md)에 구분한다. 기본 모습이 플레이 대상이고 추가 모습 자료는 열람용이다.
...
- 성도 또는 관동 스타터를 선택한다. 성도 리그 → 관동 리그 → 호연 → 신오 → 하나 → 칼로스 → 알로라 순서로 진행한다. 알로라 진행은 시련 인증으로 표시한다.
