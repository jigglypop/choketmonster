...
현재 버전은 로그인 UI 없이 진행과 개체별 신경 기억을 이 기기의 IndexedDB에 저장한다. 실제 166,700-node 회로 계산은 로그인 없이 Rust에서 수행하며 정상 전투·저장은 PostgreSQL에 접근하지 않는다. 임시 서버 캐시가 사라지면 기기의 체크포인트와 짧은 이력으로 복원한다. 기존 계정 데이터는 보존한다. 이전 `server/*.ts` SQLite 구현은 참고용이다.
...
- 151종은 배지 없이 출현 목록에 포함된다. 시작 지점에는 낮은 레벨의 개체가 등장하고 외곽으로 갈수록 강해진다. 별도 트레이너 NPC나 체육관을 필수 진행으로 두지 않는다.
