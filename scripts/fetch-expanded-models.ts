...
function makeCatalog(entries: ModelEntry[], missingIds: number[], inspections: Inspection[]) {
  ...
  return `// Generated by scripts/fetch-expanded-models.ts. Do not edit by hand.\n` +
    `import { HOME_MODEL_COMMIT, HOME_MODEL_REPOSITORY, HOME_RUNTIME_MODEL_IDS, HOME_RUNTIME_MODEL_SOURCES } from './pokemon-home-runtime-sources';\n` +
    `export type PokemonModelSource = { id: number; url: string; format: 'glb'; requiresDraco: boolean; compression?: 'KHR_draco_mesh_compression'; textureEncoding?: 'webp'; repository: string; commit: string; sourcePath: string; bytes?: number; sourceClass?: 'primary'|'research-fallback'; rightsStatus?: 'source-license-unverified' };\n\n` +
    ...
    `export function hasPokemonModel(id: number): boolean { return Number.isInteger(id) && id >= 1 && (id <= 151 || EXPANDED_MODEL_SET.has(id) || HOME_RUNTIME_MODEL_SET.has(id)); }\n` +
    ...
    `  const home = HOME_RUNTIME_MODEL_SOURCES[id];\n` +
    `  if (home) return { id, url: home.url, format: 'glb', requiresDraco: false, repository: HOME_MODEL_REPOSITORY, commit: HOME_MODEL_COMMIT, sourcePath: home.sourcePath, bytes: home.bytes, sourceClass: 'research-fallback', rightsStatus: 'source-license-unverified' };\n` +
...
async function main() {
  ...
  const manifest = {
    ...
    alternativesReviewed: [
      ...
      { repository: 'https://github.com/Lilothestitch16/Pokemon-HOME-GLB-Models', commit: '27703273836f38f0e185976d955b1fbfb15448af', result: 'All 54 primary-source gaps were inspected locally, rigged, and given explicitly authored sprite-derived flat materials. Runtime resolves pinned owner URLs without copying GLBs into public/dist. The repository has no license, so sourceClass and rightsStatus remain explicit and no redistribution-rights claim is made.' },
      ...
      { repository: 'https://github.com/Naaylla/pokemons-3D', commit: '9a723e37ab4341a5c9d67390ad49bc94812f4131', result: 'The complete pinned tree contains one Bulbasaur GLB and no license; it fills none of the 54 gaps.' },
      { repository: 'https://github.com/Lilothestitch16/Pokemon-HOME-Unity-Models', commit: '7b18d1a3e22df48329220ea99c4d2a6617d72345', result: 'The repository is an unlicensed extracted Unity project. Its recursive tree is too large for the GitHub tree API and exposes no GLB, glTF, FBX, or OBJ files in the returned 49,616 blobs; it is not an attributable runtime GLB source.' },
    ],
    catalog: { nationalRange: [1, 1025], legacySpecies: 151, expandedSpecies: supported.length, totalSupportedSpecies: 151 + supported.length, missingSpecies: missingIds.length, missingIds, totalPrimarySpecies: 151 + supported.length, primaryMissingSpecies: missingIds.length, primaryMissingIds: missingIds, externalFallbackSpecies: 54, totalRuntimeSpecies: 1025, regularFiles: regular.length, regularBytes: regular.reduce((sum, entry) => sum + (entry.size ?? 0), 0), expandedEntries: supported },
