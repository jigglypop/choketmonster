...
function parseArgs() {
  ...
  return { ids: ids.sort((a, b) => a - b), catalogOnly: process.argv.includes('--catalog-only') };
}
...
async function inspectGlb(id: number, path: string): Promise<Inspection> {
  ...
  if (!draco || !extensionsUsed.includes('KHR_draco_mesh_compression')) throw new Error(`${id}: expected Draco compression missing`);
  if (!webpTextures) throw new Error(`${id}: expected embedded WebP textures missing`);
  return {
...
function makeCatalog(entries: ModelEntry[], missingIds: number[]) {
  ...
  return `// Generated by scripts/fetch-expanded-models.ts. Do not edit by hand.\n` +
    ...
    `  return { id, url: \`https://raw.githubusercontent.com/Pokemon-3D-api/assets/\${EXPANDED_MODEL_COMMIT}/\${sourcePath}\`, format: 'glb', requiresDraco: true, compression: 'KHR_draco_mesh_compression', textureEncoding: 'webp', repository: '${REPOSITORY}', commit: EXPANDED_MODEL_COMMIT, sourcePath, bytes: EXPANDED_MODEL_BYTES[id] };\n` +
    `}\n`;
}

async function main() {
  const { ids, catalogOnly } = parseArgs();
  ...
  const selected = ids.map(id => { const entry = supported.find(item => item.id === id); if (!entry) throw new Error(`model ${id} is absent at pinned commit`); return entry; });
  const inspections: Inspection[] = [];
  if (!catalogOnly) for (const entry of selected) inspections.push(await inspectGlb(entry.id, await download(entry)));
  const previousSample = catalogOnly && await exists(MANIFEST_OUTPUT)
    ? (JSON.parse(await readFile(MANIFEST_OUTPUT, 'utf8')) as { sample?: { requestedIds: number[]; totalBytes: number; inspections: Inspection[] } }).sample
    : undefined;
  const sample = previousSample ?? { requestedIds: ids, totalBytes: selected.reduce((sum, entry) => sum + entry.bytes, 0), inspections };
  const catalog = makeCatalog(supported, missingIds); await writeFile(CATALOG_OUTPUT, catalog);
  ...
  console.log(`Cataloged ${supported.length} post-Kanto species; ${missingIds.length} missing; inspected ${inspections.length} samples this run (${sample.totalBytes} audited bytes).`);
}
