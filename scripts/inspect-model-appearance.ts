...
const ids = process.argv[2] === 'review-all'
...
try {
  ...
  const result = await page.evaluate(async id => {
    ...
    return inspectAppearance(`/models/pokemon/${id}.glb`, id, true);
  }, id);
  await page.screenshot({ path: `${output}/render.png` });
  await writeFile(`${output}/report.json`, JSON.stringify({ ...result, errors }, null, 2));
  console.log(JSON.stringify({ id, report: result.report, errors, gpuErrors: result.gpuErrors }));
