...
for(const [region,[first,last]] of Object.entries(ranges) as Array<[Region,readonly [number,number]]>) for(let id=first;id<=last;id++) results.push({...inspect(id),region});
...
const output={schema:1,generatedAt:new Date().toISOString(),source:{repository:manifest.source.repository,commit,rights:manifest.rights},scope:{nationalDex:[252,649],species:results.length,bytes:results.reduce((sum,row)=>sum+row.bytes,0)},summary,results};
