...
async function importRegion(page:Page,region:'galar'|'hisui'|'paldea',partner:number){
  ...
  game.player.team=[createMonster(game,partner,35)];game.dex.caught=[152,partner].sort((a,b)=>a-b);game.dex.seen=[...game.dex.caught];
...
for(const [region,partner,source] of cases)test(`${region} loads exact ${source} partner, walks by map, and restores the model`,async({page},testInfo:TestInfo)=>{
  ...
  await page.locator('[data-tab="map"]').click();await page.locator('#world-map-open').click();await expect(page.locator('#world-map-dialog')).toBeVisible();
  ...
  expect(target).toBeDefined();await page.locator(`[data-location-id="${target.id}"] circle`).click();await expect(page.locator('#world-map-dialog')).not.toBeVisible();await expect(page.locator('#world-position')).not.toHaveText(before,{timeout:20_000});
