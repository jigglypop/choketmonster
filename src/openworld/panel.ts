...
export class OpenWorldPanel {
  ...
  mount(host: HTMLElement): void {
    ...
    const orientMap = (orientation: MapOrientation) => {
    ...
    this.host.querySelectorAll<HTMLButtonElement>('[data-map-orientation]').forEach(button => button.onclick = () => orientMap(button.dataset.mapOrientation as MapOrientation));
    // The map turns a quarter at a time with the camera, like the compass buttons.
    const turns: readonly MapOrientation[] = ['north', 'east', 'south', 'west'];
    const turnMap = (step: number) => orientMap(turns[(turns.indexOf(nearestMapOrientation(this.cameraHeading)) + step + 4) % 4]);
    this.button('#world-map-rotate-left').onclick = () => turnMap(-1);
    this.button('#world-map-rotate-right').onclick = () => turnMap(1);
