...
export class OpenWorldPanel {
  ...
  mount(host: HTMLElement): void {
    ...
    host.innerHTML = `<section class="adventure" aria-label="오픈월드 모험">
      ...
      <details class="world-multiplayer"><summary><span id="world-realtime-dot"></span><strong id="world-realtime-status">실시간 연결 중</strong><small id="world-realtime-count">0명 · —ms</small></summary><p>같은 지역 플레이어 · 개인 모험 진행 유지</p><label>표시 이름 <input id="world-realtime-name" autocomplete="nickname"></label><button id="world-realtime-rename">이름 적용</button><div id="world-player-list" aria-label="같은 지역 플레이어"></div><div id="world-chat-log" role="log" aria-live="polite"></div><form id="world-chat-form"><input id="world-chat-input" autocomplete="off" placeholder="같은 지역에 메시지 보내기"><button>전송</button></form><small id="world-chat-error"></small></details>
      <div class="world-lower-hud">
        ...
        <details class="world-battle-hud" aria-label="파트너와 배틀">
          ...
          <div class="world-battle-deck">
            ...
            <label class="world-exp-share" title="대기 팀원 80% · 출전 개체보다 레벨이 낮으면 100%"><input id="world-exp-share" type="checkbox"><span>팀 경험치 공유<small>대기 팀원 80% · 출전 개체보다 낮으면 100%</small></span></label>
    ...
    chat.addEventListener('submit', event => { event.preventDefault(); if (composing) return; if (this.multiplayer?.sendChat(chatInput.value)) { chatInput.value = ''; this.html('#world-chat-error', ''); } else this.html('#world-chat-error', '연결 후 1~200자로 보내 주세요.'); });
    ...
    versionSelect.onchange = () => {
      try { this.simulation.changeVersion(versionSelect.value); this.options.changed(); this.refresh(); this.options.notify(`${this.simulation.atlas.name} 수집 기록을 변경했습니다. 해당 지역의 고정 야생 배치를 유지합니다.`); }
  ...
  private presence() {
    ...
    return { region: this.simulation.regionId as 'kanto' | 'johto', speciesId: lead.speciesId, x: player.x, z: player.z, heading: player.heading,
  ...
  private renderRealtime(): void {
    ...
    const view = this.multiplayer.view;
    this.html('#world-realtime-status', view.status === 'connected' ? '실시간 연결됨' : view.status === 'reconnecting' ? '다시 연결 중' : view.status === 'connecting' ? '연결 중' : '오프라인');
  ...
  refresh(): void {
    ...
    const pool = regionalEncounters(location.id, getRegionalBadges(game, world.regionId), world.regionId);
  ...
  private drawRegionMap(): void {
    ...
    this.html('#world-map-content', `<svg viewBox="0 0 260 240" role="img" aria-label="${world.atlas.name} 도시 연결 지도"><rect width="260" height="240" rx="8" fill="#bed5c0"/><g stroke="#faf1ce" stroke-width="3" fill="none">${lines}</g><g fill="#35594a">${towns}</g>${destination ? `<circle cx="${scale(destination.x)}" cy="${scale(destination.z)}" r="5" fill="#f5c542" stroke="#5b4313"/><text x="${scale(destination.x) + 6}" y="${scale(destination.z) + 7}">다음</text>` : ''}<circle cx="${scale(world.player.x)}" cy="${scale(world.player.z)}" r="4" fill="#e04f45" stroke="white"/><text x="8" y="16">N ↑ · 빨간 점: 내 위치 · 노란 점: 다음 목적지</text></svg><div class="kanto-zone-list">${this.simulation.atlas.locations.map(item => { const levels = regionalWildLevels(game, region, item); return `<div class="${this.simulation.locationAt(world.player.x, world.player.z).id === item.id ? 'current' : ''}${destination?.id === item.id ? ' destination' : ''}"><strong>${item.name}${destination?.id === item.id ? ' · 다음 목적지' : ''}</strong><small>${item.encounters.length ? `Lv.${levels.minLevel}–${levels.maxLevel} · ${item.encounters.slice(0, 4).map(id => getSpecies(id).name).join(' / ')}` : '마을 · 연결 거점'}${item.requiredBadges ? ` · 배지 ${item.requiredBadges}개` : ''}</small></div>`; }).join('')}</div>`);
  }
