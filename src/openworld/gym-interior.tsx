...
export function GymInterior({ hall, gym, party, badges, busy, player, spriteUrl, onNavigate, onExit, onChallenge }: {
  ...
  return <group name={`gym-interior:${hall.locationId}`} dispose={null}>
    ...
    {[[0, -courtDepth / 2, courtWidth, .16], [0, courtDepth / 2, courtWidth, .16], [0, 0, courtWidth, .12], [-courtWidth / 2, 0, .16, courtDepth], [courtWidth / 2, 0, .16, courtDepth]].map(([x, z, w, d], index) =>
      <mesh key={index} position={[cx + x, y + .012, courtZ + z]} rotation={[-Math.PI / 2, 0, 0]} material={materials.line} receiveShadow><planeGeometry args={[w, d]} /></mesh>)}
    <mesh position={[cx, y + .014, courtZ]} rotation={[-Math.PI / 2, 0, 0]} material={materials.line} receiveShadow><ringGeometry args={[2.3, 2.46, 48]} /></mesh>
    <mesh position={[cx, y + .014, courtZ]} rotation={[-Math.PI / 2, 0, 0]} material={materials.line} receiveShadow><circleGeometry args={[.42, 24]} /></mesh>
    ...
    {[-1, 1].map(side => <group key={`front:${side}`} position={[cx + side * (DOOR_WIDTH / 2 + sideWall / 2), y, hall.minZ]}>
      ...
      <mesh position={[0, FRONT_HEIGHT + .06, 0]} material={materials.stripe} castShadow receiveShadow><boxGeometry args={[sideWall, .12, WALL_THICKNESS + .06]} /></mesh>
    </group>)}
    {[-1, 1].map(side => <mesh key={`door:${side}`} position={[cx + side * (DOOR_WIDTH / 2 + .25), y + 1.6, hall.minZ]} material={materials.pillar} castShadow receiveShadow><boxGeometry args={[.5, 3.2, WALL_THICKNESS + .1]} /></mesh>)}
    {[-1, 1].map(side => <mesh key={`stripe:${side}`} position={[cx + side * (width / 2 - WALL_THICKNESS / 2 - .02), y + 1.1, cz]} material={materials.stripe} receiveShadow><boxGeometry args={[.06, .5, depth - 1]} /></mesh>)}
    <mesh position={[cx, y + 1.1, hall.maxZ - WALL_THICKNESS / 2 - .02]} material={materials.stripe} receiveShadow><boxGeometry args={[width - 1, .5, .06]} /></mesh>

    {pillars.map((pillar, index) => <group key={index} position={[pillar.x, y, pillar.z]}>
      <mesh position={[0, .16, 0]} material={materials.dais} castShadow receiveShadow><boxGeometry args={[1.2, .32, 1.2]} /></mesh>
      <mesh position={[0, 2.35, 0]} material={materials.pillar} castShadow receiveShadow><cylinderGeometry args={[.42, .5, 4.1, 16]} /></mesh>
      <mesh position={[0, 4.36, 0]} material={materials.stripe} castShadow receiveShadow><cylinderGeometry args={[.56, .44, .2, 16]} /></mesh>
    ...
    <group position={[cx, y, hall.leader.z]}>
      ...
      <mesh position={[0, .1, -2.25]} material={materials.dais} castShadow receiveShadow><boxGeometry args={[3.2, .2, .9]} /></mesh>
    ...
    {[-1, 0, 1].map(offset => <mesh key={offset} position={[cx + offset * 4.2, y + 3.6, hall.maxZ - WALL_THICKNESS / 2 - .04]} material={offset ? materials.stripe : materials.dais} receiveShadow>
      <planeGeometry args={[offset ? 1.6 : 2.4, offset ? 3.6 : 4.4]} />
    </mesh>)}

    {/* Exit mat just inside the door. */}
    <group position={[hall.exit.x, y, hall.exit.z]}>
      <mesh position={[0, .03, 0]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow onClick={event => { event.stopPropagation(); if (exitDistance <= 2.4) onExit(); else onNavigate(hall.exit); }}>
