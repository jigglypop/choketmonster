...
export const JOHTO_CONNECTIONS: ReadonlyArray<readonly [string, string]> = [
  ['new-bark', 'route-27'], ['route-27', 'tohjo-falls'],
...
// Johto badges are not represented independently in the current save schema.
// Keep exploration separate from Kanto badge awards and gates.
...
export function sampleJohtoWorld(x: number, z: number): WorldSample {
  const height = .2 * Math.sin((x + 11) * .085) + .16 * Math.cos((z - 7) * .075);
  ...
  if (town) return { height, biome: 'meadow', blocked: townBuildingAt(x, z, town) };
  ...
  if ((westernSea || rageLake) && !playable) return { height: -.7, biome: 'lake', blocked: true };
  if (pathDistance < 3.2) {
    ...
    return { height: pathHeight, biome: pathBiome === 'sea' ? 'lake' : pathBiome === 'cave' ? 'rock' : pathBiome === 'forest' ? 'forest' : 'meadow', blocked: false };
  }
  if (nearest.kind === 'sea' && playable) return { height: -.66, biome: 'lake', blocked: false };
  if (playable) {
    const rocky = nearest.kind === 'cave' || nearest.id === 'route-44' || nearest.id === 'route-45';
    return { height: rocky ? height + .58 : height, biome: rocky ? 'rock' : nearest.kind === 'forest' ? 'forest' : 'meadow', blocked: false };
  }
  const mountain = x > 42 || z < -32;
  return { height: height + (mountain ? .72 : .18), biome: mountain ? 'rock' : 'forest', blocked: true };
}
...
export function safeJohtoArrival(locationId: string): { x: number; z: number } | undefined {
...
export function nearestJohtoWalkable(x: number, z: number, badges = 0): { x: number; z: number } | undefined {
  ...
  if (isJohtoPlayable(x, z)) return { x, z };
  for (let radius = .5; radius <= 18; radius += .5) for (let step = 0; step < 32; step += 1) {
    ...
    if (isJohtoPlayable(candidate.x, candidate.z)) return candidate;
  }
  const fallback = [...JOHTO_LOCATIONS].sort((a, b) => Math.hypot(a.x - x, a.z - z) - Math.hypot(b.x - x, b.z - z))[0];
  return fallback ? safeJohtoArrival(fallback.id) : undefined;
}
...
export function johtoEncounters(locationId: string, badges: number): number[] {
  ...
  return found && Number.isFinite(badges) && badges >= 0 ? [...found.encounters] : [];
}
