import { getPokemonModelSource } from '../data/pokemon-models';
export { hasPokemonModel } from '../data/pokemon-models';
...
export function pokemonModelUrl(id: number): string {
  if (id > 151) return getPokemonModelSource(id)?.url ?? `/models/pokemon/${id}.glb`;
...
export function pokemonSpriteUrl(id: number | string, back = false): string {
