...
export function mountAccountPanel(options: AccountPanelOptions) {
  ...
  // The dialogs open on their own (a save conflict, an expired session) while the header's account menu may be closed,
  // and a closed <details> draws nothing inside it: a modal there would lock the whole page out of sight.
  const layer = document.body.appendChild(document.createElement('div'));
  layer.className = 'account-dialogs';
  ...
  const dialog = layer.querySelector<HTMLDialogElement>('.account-dialog')!, form = dialog.querySelector<HTMLFormElement>('form')!;
  const open = host.querySelector<HTMLButtonElement>('[data-open-auth]')!, logoutButton = host.querySelector<HTMLButtonElement>('.logout-button')!, name = host.querySelector<HTMLElement>('.account-name')!, error = dialog.querySelector<HTMLElement>('.account-error')!;
  ...
  const recoveryDialog = layer.querySelector<HTMLDialogElement>('.save-recovery-dialog')!, recoveryError = recoveryDialog.querySelector<HTMLElement>('.account-error')!;
  ...
  const setBusy = (value: boolean) => {
    busy = value; for (const button of [...host.querySelectorAll<HTMLButtonElement>('button'), ...layer.querySelectorAll<HTMLButtonElement>('button')]) button.disabled = value;
  ...
  return {
    ...
    destroy() { disposed = true; unsubscribe(); unsubscribeStorage(); stopAutosave(); layer.remove(); host.querySelector('.account-panel')?.remove(); },
  };
}
