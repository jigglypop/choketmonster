...
function renderSelectedDetail() {
  ...
  if (selected.speciesId <= 151) getPokemonScene().showSpecimen($('.detail-portrait'), selected.speciesId); else getPokemonScene().detach();
}
...
function renderDex() {
  ...
  $('#screen').innerHTML = `<div class="page dex-page"><section class="section-heading"><div><span class="kicker">POKÉDEX · COLLECTION</span><h1>${escapeHtml(versionName)}</h1><p>수록 ${pool.length}종 · 수집 ${caughtIds.filter(id => pool.some(s => s.id === id)).length}종</p></div>
    ...
    <section class="collection-note"><p>버전 목록은 원본 지역도감 기준입니다. 공용 관동 맵에 이 게임의 규칙으로 배치하며, 본가의 출현 위치·버전 독점 조건과 다릅니다. 버전마다 직접 잡거나 진화한 기록을 따로 모읍니다.</p><button id="collect-version" class="primary" ${game.adventureVersion === dexVersion ? 'disabled' : ''}>${game.adventureVersion === dexVersion ? '이 버전 수집 중' : '이 버전에서 수집'}</button></section>
    <div class="dex-grid">${filtered.slice(dexPage * pageSize, (dexPage + 1) * pageSize).map(s => {
      ...
      const regions = s.id <= 151 && ['red', 'blue', 'yellow'].includes(dexVersion) ? kantoSpeciesSources(s.id).join(' / ') : '수집 버전 선택 후 공용 맵 탐험';
...
function showModel(id: number) {
  ...
  if (id <= 151) getPokemonScene().showSpecimen(dialog.querySelector<HTMLElement>('.model-host')!, id); else { dialog.querySelector('.model-host')!.innerHTML = `<img class="species-preview" src="${species.frontSprite}" alt="${escapeHtml(species.name)}">`; dialog.querySelector('.model-control-hint')!.textContent = '원본 스프라이트 · 이 종은 2D 이미지로 표시됩니다.'; }
