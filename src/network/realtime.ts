...
const validPlayer = (value: unknown): value is RemotePlayer => { const p = value as RemotePlayer; return Boolean(p && typeof p.id === 'string' && p.id && typeof p.name === 'string' && codePointLength(p.name) <= 32 && validRegion(p.region) && validScene(p.sceneId, p.region) && Number.isInteger(p.speciesId) && p.speciesId > 0 && p.speciesId <= 1_025 && [p.x, p.z, p.heading, p.updatedAt].every(Number.isFinite) && ['idle', 'moving', 'battle'].includes(p.activity)); };
